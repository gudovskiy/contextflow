...
from layers.flowlayer import FlowLayer, mark_expensive, ModifiedGradFlowLayer
...
#    int[]? bias_sizes, int[] stride, int[] padding, int[] dilation, bool transposed, int[] output_padding, int groups, bool[3] output_mask) -> (Tensor, Tensor, Tensor)
# output_mask: 3-dim boolean array specifying which gradients to compute in input, weight, bias order

#@lru_cache(maxsize=128)
#def _compute_weight_multiple(W, output, x, padding, stride, dilation, groups, benchmark, deterministic):
#
#    #op = torch._C._jit_get_operation('aten::grid_sampler_2d_backward')
#    batch_multiple = torch.ops.aten.convolution_backward(torch.ones_like(output), torch.ones_like(x), W, None, stride, output_padding, dilation, False, padding, groups, [False, True, False])[1]
#    #batch_multiple = conv2d_backward.backward_weight(wshape, torch.ones_like(output), torch.ones_like(x), padding, stride, dilation, groups, benchmark, deterministic)
#    return batch_multiple / len(x)


#def flip_kernel(W): return torch.flip(W, (2,3)).permute(1,0,2,3).clone()
flip_kernel = lambda W: rearrange(W, 'c1 c2 k1 k2 -> c2 c1 k1 k2').clone()
...
class SelfNormConvFunc(autograd.Function):
    ...
    def backward(ctx, output_grad):
        ...
        output_padding = torch.Size((0,0))
        dilation = torch.Size(ctx.dilation)
        groups = ctx.groups
        benchmark = False
        deterministic = False

        #op = torch._C._jit_get_operation('aten::convolution_backward')
        #flags = [torch.backends.cudnn.benchmark, torch.backends.cudnn.deterministic, torch.backends.cudnn.allow_tf32]
        
        #multiple = _compute_weight_multiple(W, output, x, padding, stride, dilation, groups, benchmark, deterministic)
        multiple = torch.ops.aten.convolution_backward(torch.ones_like(output), torch.ones_like(x), W, None, stride, padding, dilation, False, output_padding, groups, [False, True, False])[1] / len(x)

        # Grad_W LogP(x)
        delta_z_xt = torch.ops.aten.convolution_backward(output_grad, x, W, None, stride, padding, dilation, False, output_padding, groups, [False, True, False])[1]
        #delta_z_xt = conv2d_backward.backward_weight(W.shape, output_grad, x, padding, stride, dilation, groups, benchmark, deterministic)
        
        weight_grad_fwd = (delta_z_xt - flip_kernel(R) * multiple) / 2.0

        # Grad_R LogP(x)
        input_grad = torch.ops.aten.convolution_backward(output_grad, x, W, None, stride, padding, dilation, False, output_padding, groups, [True, False, False])[0]
        #input_grad = conv2d_backward.backward_input(x.shape, output_grad, W, padding, stride, dilation, groups, benchmark, deterministic)

        Wx = output - bw.view(1, -1, 1, 1) if bw is not None else output
        
        neg_delta_x_Wxt = torch.ops.aten.convolution_backward(-1*input_grad, Wx, R, None, stride, padding, dilation, False, output_padding, groups, [False, True, False])[1]
        #neg_delta_x_Wxt = conv2d_backward.backward_weight(R.shape, -1*input_grad, Wx, padding, stride, dilation, groups, benchmark, deterministic)
        
        weight_grad_inv = (neg_delta_x_Wxt + flip_kernel(W) * flip_kernel(multiple)) / 2.0

        if bw is not None:  # Sum over all except output channel
...
class SelfNormConv(ModifiedGradFlowLayer):
    def __init__(self,
        ...
        if self.context_net:
            self.context_size = context_net.size
            self.in_channels += self.context_size
        else: self.context_size = 0
    ...
    def reset_parameters(self):
        ...
        if self.kernel_size[0] == 1 and self.kernel_size[1] == 1 and self.out_channels == self.in_channels:
        ...
        b_small = torch.nn.init.normal_(torch.empty(self.out_channels), std=w_noise.std())
        self.bias_fwd = nn.Parameter(b_small) if self.use_bias else None
        print('weight_fwd:', self.weight_fwd.shape)
        print('weight_inv:', self.weight_inv.shape)
    ...
    def forward(self, input, context=None, compute_expensive=False):
        ...
        if self.context_net:
            input = torch.cat([input, self.context_net(context)], dim=1)

        self.input = input

        if compute_expensive:
            self.output = F.conv2d(input, self.weight_fwd, self.bias_fwd, self.stride, self.padding, self.dilation, self.groups)
            ldj = self.logdet(input)
        else:
            self.output = selfnorm_conv_2d(input, self.weight_fwd, self.bias_fwd, self.weight_inv, self.stride, self.padding, self.dilation, self.groups)
            ldj = torch.zeros(input.shape[0]).to(device=input.device)
    ...
    def reverse(self, input, context=None, compute_expensive=False):
        ...
        if compute_expensive:
            ...
            rev = torch.matmul(T_sparse.to_dense().inverse().to(input.device), input.flatten(start_dim=1).unsqueeze(-1))
            rev = rev.view(input.shape)
        else:
            ## Use inverse weights
            rev = F.conv2d(input, self.weight_inv, None, self.stride, self.padding, self.dilation, self.groups)
        return rev

    def add_recon_grad(self, recon_loss_weight_update=None):
        # Compute ||x - RWx||^2
        x = self.input.detach() # have to compute z again w/ detached x
        z = F.conv2d(x, self.weight_fwd, None, self.stride, self.padding, self.dilation, self.groups)
        if self.only_R_recon: z = z.detach()
        x_hat = F.conv2d(z, self.weight_inv, None, self.stride, self.padding, self.dilation, self.groups)
        recon_loss = (x - x_hat).pow(2).flatten(start_dim=1).sum(-1)

        # Compute ||z - WRz||^2
        if self.sym_recon_grad:
            zsym = z.detach()
            xsym      = F.conv2d(z,    self.weight_inv, None, self.stride, self.padding, self.dilation, self.groups)
            z_hat_sym = F.conv2d(xsym, self.weight_fwd, None, self.stride, self.padding, self.dilation, self.groups)
    ...
    def sparse_toeplitz(self, input):
        if self.T_idxs is None or self.f_idxs is None:
            self.T_idxs, self.f_idxs = get_toeplitz_idxs(self.weight_fwd.shape, input.shape[1:], self.stride, self.padding)

        T_sparse = get_sparse_toeplitz(self.weight_fwd, input.shape[1:], self.T_idxs, self.f_idxs)
        return T_sparse
    ...
    def logdet(self, input):
        if self.logabsdet_dirty:
            print(input.shape)
            T_sparse = self.sparse_toeplitz(input)
