...
class UniformDistribution(nn.Module):
    ...
    def log_prob(self, input, context=None):
        ...
        log_px = torch.where(above_0 & below_1, torch.zeros_like(input), torch.ones_like(input) * minus_infty)
        log_px = log_px.view(log_px.size(0), self.dim).sum(-1)

        return log_px

    def sample(self, n_samples, context=None):
        x = torch.rand((n_samples, *self.size)).to(device=self.empty.device) / self.scale
        log_px = torch.zeros(x.shape[0]).to(device=self.empty.device)
        return x, log_px
