...
def _initialize_embeddings(weight: Tensor, d: int, init='uniform') -> None:
...
class OneHotEncoder(nn.Module):
    ...
    * **Input data type**: ``integer``
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        embedding = torch.cat(encoded_columns, 1)
        context = x
        return embedding, context
    ...
    * **Input data type**: ``integer``
    ...
    def reset_parameters(self) -> None:
        for module in self._embeddings:
            _initialize_embeddings(module.weight, module.weight.shape[-1], init=self.init)
        if self._biases is not None:
            for x in self._biases:
                _initialize_embeddings(x, x.shape[-1], init=self.init)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        embedding = torch.stack(out, 1) if self.stack else torch.cat(out, 1)
        context = x
        return embedding, context
...
class PiecewiseLinearEncoder(nn.Module):
    ...
    def forward(self, x: Tensor, indices: Optional[Tensor] = None) -> Tensor:
        ...
        if indices is None:
            ...
            embedding = compute_piecewise_linear_encoding(x, bin_edges, stack=self.stack)
        else:
            ...
            ratios = x
            embedding = piecewise_linear_encoding(bin_edges, ratios, indices, self.d_encoding, stack=self.stack)
...
class PeriodicEmbeddings(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        #print(x.shape, self.coefficients.shape)
        if x.ndim != 2: raise ValueError('The input must have two dimensions')
        #print(x.shape, self.coefficients.shape)
        y = 2 * math.pi * self.coefficients[None] * x[..., None]
        embedding = torch.cat([torch.cos(y), torch.sin(y)], -1)
        #context = x
        return embedding #, context
...
class NLinear(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if x.ndim != 3: raise ValueError('The input must have three dimensions (batch_size, n_tokens, d_embedding)')
        y = x[..., None] * self.weight[None]
        y = y.sum(-2)
        if self.bias is not None:
            y = y + self.bias[None]
...
def make_plr_embeddings(
    ...
    * (R) ReLU activation
    ...
    Args:
        ...
        sigma: the **super important** parameter for `Periodic`
    Returns:
        embeddings: the embedding module
